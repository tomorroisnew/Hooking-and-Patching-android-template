...
echo.
if %archt%==1 (
adb push %pp%\x32\libs\armeabi-v7a\libyeet.so /data/local/tmp
) else if %archt%==2 (
adb push %pp%\x64\libs\arm64-v8a\libyeet.so /data/local/tmp
)
frida -U -f game --no-pause -l %pp%\Loader.ts